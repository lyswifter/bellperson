...
fn create_fft_kernel<'a, F>(priority: bool, idx: Option<usize>) -> Option<FftKernel<'a, F>>
where
    F: Field + GpuName,
{
...
fn create_multiexp_kernel<'a, G>(priority: bool, idx: Option<usize>) -> Option<CpuGpuMultiexpKernel<'a, G>>
where
        ...
        )+
        {
            ...
            fn init(&mut self) {
                if self.kernel_and_lock.is_none() {
                    ...
                    if let Some(kernel) = $func(self.priority, self.idx) {
